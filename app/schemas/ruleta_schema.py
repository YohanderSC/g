...
class RuletaBase(BaseModel):
    nombre: str = Field(..., min_length=3, max_length=150, description="Nombre del evento de ruleta")
    descripcion: Optional[str] = Field(None, description="Descripción opcional del evento")
    fecha_inicio: datetime = Field(..., description="Fecha y hora de inicio del evento")
    fecha_cierre: Optional[datetime] = Field(None, description="Fecha y hora de cierre del evento")
    activa: bool = Field(True, description="Si la ruleta está habilitada para participar")
    max_giros: int = Field(1, ge=1, description="Máximo de giros permitidos por cliente")
...
class RuletaListResponse(BaseModel):
    ...
    datos: list[RuletaResponse] = Field(..., description="Lista de ruletas en la página actual")
